...
public class Hotel implements Serializable {
    ...
    // Create a booking
    public Booking makeBooking(String roomType, int roomNumber, int numberOfNights, LocalDate startDate, LocalDate endDate, Guest guest) {
    ...
    // CHANGE STAFF INFO
    public Hotel changeStaffInfo(int phoneNumber, String firstName, String LastName, String title,  double Salary) {
    ...
    //register NEW STAFFAccount
    public Staff registerCleaningLady(String title, String firstname, String Lastname, int phoneNumnber, double salery) {
        StaffCleaningLady newStaffC = new StaffCleaningLady(title, firstname, Lastname, phoneNumnber, salery);
        listOfStaff.add(newStaffC);
        return newStaffC;
    }
    //register NEW STAFFAccount
    public StaffUser registerNewDirector(String title, String firstname, String Lastname, int phoneNumnber, double salery, String password, String username) {
    ...
    //register NEW STAFFAccount
    public StaffUser registerNewReception(String title, String firstname, String Lastname, int phoneNumnber, double salery, String password, String username) {
        StaffReceptionist newStaffD = new StaffReceptionist(title, firstname, Lastname, phoneNumnber, salery, password, username);
    ...
    public boolean availableDoubleBed1(int numberOfGusts, boolean isOccupied) {
    ...
    //update list of rooms after booking and add check in and check out date

    public void updateListOfRooms(int roomNumber) {
        for (int i = 0; i < listOfRooms.size(); i++) {
            if (listOfRooms.get(i).getRoomNumber() == roomNumber) {
    ...
    public void showListOfRooms() {
        for (int i = 0; i < listOfRooms.size(); i++) {
            if (listOfRooms.get(i).getIsOccupied() == false) {
                ...
                System.out.println(listOfRooms.get(i).getRoomType() + " |ROOM NUMBER " + listOfRooms.get(i).getRoomNumber()+ " |IS OCCUPIED "+ translate2(listOfRooms.get(i).getIsOccupied()));
            }
        }
    }
